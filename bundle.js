...
function draw(state) {
  var draw_one = function (i, j, e) {
    var dot_w = div(canvas.width, state.size.x);
    var dot_h = div(canvas.height, state.size.y);
  ...
  if (state.size.x !== 0 && state.size.y !== 0) {
    return lmatrix_iterij(draw_one, state.board);
  } else {
    return /* () */0;
  }
}
  ...
  if (typeof $$event === "number") {
    switch ($$event) {
      case /* Nothing */0 :
          ...
          break;
      case /* Next */1 :
          board = clamp(next(state.board));
          break;
      case /* Previous */2 :
          board = hd(state.previous);
          break;
      case /* Reset */3 :
          board = lmatrix_create(3, 3, /* Dead */0);
          break;
